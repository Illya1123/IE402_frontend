...
const Tours = () => {
  ...
  const allSelected = tourData.every((row) => selectedRows[row.id]);
  const someSelected = tourData.some((row) => selectedRows[row.id]);
  ...
  useEffect(() => {
    const filtered = tourData.filter((row) =>
  ...
  useEffect(() => {
    const selectedStatuses = tourData
  ...
  const handleUpdateStatus = () => {
    const updatedData = tourData.map((row) =>
  ...
  return (
    <div className="flex w-full flex-wrap p-4">
      <Header title="Quản lý Chuyến Đi" />
      ...
      <div className="w-full">
        ...
        <div className="mb-10">
          <input
            type="text"
            placeholder="Tìm kiếm thông tin Chuyến đi ..."
        ...
        <div className="mb-4 flex gap-2 p-2">
          <span className="text-black">Cập nhật tình trạng chuyến đi: </span>
          <select
          ...
          >
            ...
            <option value="Chờ đặt vé">Chờ đặt vé</option>
          </select>
          <button
                  ...
                  {Object.entries(row)
                    ...
                    .map(([key, value]) => (
                      <td
                        key={key}
                        className={`border-b-2 border-b-gray-200 p-2 py-4 text-center ${
                          row.column6 === "Chờ đặt vé"
                            ? "text-blue-500"
                            : row.column6 === "Đầy chỗ"
                              ? "text-red-500"
                              : row.column6 === "Đang thực hiện"
                                ? "animate-blink-delay text-green-500"
                                : ""
                        } ${selectedRows[row.id] ? "bg-yellow-100" : ""}`}
                      >
                        {value}
